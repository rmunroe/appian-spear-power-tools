...
@SptListCategory
public class SPT_List_LeastFrequent {
    @Function
    public TypedValue spt_list_leastfrequent(
            TypeService ts,
        ...
        Integer bottomCount = Collections.min(map.entrySet(), Map.Entry.comparingByValue()).getValue();

        List<Object> keys = new ArrayList<>();
        for (Map.Entry<Object, Integer> entry : map.entrySet())
            if (entry.getValue().equals(bottomCount)) keys.add(entry.getKey());
